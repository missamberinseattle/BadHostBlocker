...
namespace BadHostBlockerTests
{
    ...
    public class UnitTests : AssertionHelper
    {
        ...
        {
            var senders = new[] {
                new {mailFrom = "support@boldtypetickets.com",  from = "Support <support@boldtypetickets.com>", expected = PostProcessResult.Accept },
                new {mailFrom = "bounce-md_30209658.5a797ca4.v1-f56f335a1b2c474896b5bd759c52d34f@mandrillapp.com", from = "William at WellAttended <william@wellattended.com>", expected = PostProcessResult.Ignore }
            };
            var result = PostProcessResult.Accept;

            foreach (var sender in senders)
            {
                BadHostPlugin.CheckMailFromWhiteList(sender.mailFrom, sender.from, "0.0.0.0", ref result);
                Assert.AreEqual(sender.expected, result, string.Format("{0} ort {1}; expected: {2}; actual: {3}",sender.mailFrom, sender.from, sender.expected, result));
            }
        ...
        public void WhiteListCheck_NotIgnore()
        {
            ...
            BadHostPlugin.CheckMailFromWhiteList("amber@clarkzoo.org", "Amber Clark <amber@clarkzoo.org>", "1.2.3.4", ref postProcessResult);
        ...
        {
            var postProcessResult = PostProcessResult.Accept;
            BadHostPlugin.CheckMailFromWhiteList("frank@clarkzoo.org", "frank@clarkzoo.org", "1.2.3.4", ref postProcessResult);
